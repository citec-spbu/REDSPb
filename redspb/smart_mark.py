...
# print(my_matrix.shape)
# view_only1(matrix)
# my_matrix[:, 3040:3060] = get_updated_matrix(matrix[:, 3040:3060], -5, 30, -5, 2, 1)
...
for i in range(len(matrix)):
    for j in range(6203, 6204):
        my_matrix[i][j] = 1
my_matrix[:, 6600:6748] = get_updated_matrix(matrix[:, 6600:6748], -15, 30, -15, 3, 1)  # done
for i in range(len(matrix)):
...
view_only1(matrix)
view_both(matrix, my_matrix)


# Запись в файл
# new_matrix = new_matrix.astype(int)
# df['target'] = new_matrix.tolist()
# df['target'] = df['target'].apply(lambda x: str(x)[1:-1])
# df[['date', 'time', 'step1', 'step2', 'step3', 'step4', 'numbers', 'target']].to_csv('data_res_2_half_upd.csv')
